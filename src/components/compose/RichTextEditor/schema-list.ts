...
/// perform the change.
export function wrapInList(listType: NodeType, attrs: Attrs | null = null, tr?: Transaction): Command {
  return function(state: EditorState, dispatch?: (tr: Transaction) => void) {
    ...
    if (dispatch) dispatch(doWrapInList(tr || state.tr, range, wrap, doJoin, listType).scrollIntoView())
    return true
  }
}
...
export function sinkListItem(itemType: NodeType): Command {
  return function(state, dispatch) {
    ...
    let parent = range.parent, nodeBefore = parent.child((startIndex - 1 ) || 1)
    // if (nodeBefore.type != itemType) {
    //   console.log('nodeBefore.type != itemType', nodeBefore.type)
    //   return false
    // }
