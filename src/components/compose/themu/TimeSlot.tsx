...
const config: Slots = {
    "80": [
        ...
        {
            id: 3,
            label: '18:00-20:00 (Group dinner)',
            from: '18:00',
            to: '20:00'
        }
    ],
    "78": [
        {
            id: 4,
            label: "8:30-11:30 (Outside events)",
            from: '8:30',
            to: '11:30'
        },
        {
            id: 5,
            label: "15:00-20:00 (Outside events)",
            from: '15:00',
            to: '20:00'
        },
        {
            id: 6,
            label: "15:00-17:00 (Outside events)",
            from: '15:00',
            to: '17:00'
        },
        {
    ...
    "81": [
        {
            id: 9,
            label: '10:30-12:00 (Workshops/Discussions/Bootcamps)',
            from: '10:30',
            to: '12:00'
        },
        {
            id: 10,
            label: '15:30-16:30 (Workshops/Discussions)',
            from: '15:30',
            to: '16:30'
        },
        {
            id: 11,
            label: '15:30-17:00 (Bootcamps)',
            from: '15:30',
            to: '17:00'
        }
    ],
    "79": [
        {
            id: 12,
            label: '10:30-12:00 (Workshops/Discussions)',
            from: '10:30',
            to: '12:00'
        },
        {
            id: 13,
            label: '15:00-15:30 Workshops/Discussions',
            from: '15:00',
            to: '15:30'
        },
    ],
    "82": [
        ...
        {
            id: 15,
            label: '15:00-16:30 (Workshops/Discussions)',
            from: '15:00',
            to: '16:30'
        },
        {
            id: 16,
            label: '15:00-17:00 (Bootcamps)',
            from: '15:00',
            to: '17:00'
        },
    ]
...
function TimeSlot(props: { eventSiteId: number, from: string, to: string, onChange?: (from: string, to: string, timezone: string) => any }) {
    ...
    useEffect(() => {
        if (props.eventSiteId) {
            ...
            const initTarget = slot1List.find((item) => {
    ...
    }, [props.eventSiteId ,props.from, props.to])
    ...
    useEffect(() => {
        if (slot && slot[0] && date) {
            ...
            const to = dayjs.tz(`${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()} ${to_str}`, timezone).toISOString()
            props.onChange && props.onChange(from, to, timezone)
        }
    }, [slot, date])
    ...
    return (
        <div className={'app-date-input-v2'}>
            <div className={'date-input'}>
                ...
                <div className={'show-date'}>{showDate(date)}</div>
                <DatePicker
                    minDate={new Date()}
                    value={date}
                    onChange={({date}) => {
            ...
            { !!slot &&
                <div className={'slots'}>
                    <Select
                        ...
                        creatable={false}
                        value={slot as any}
                        options={config[props.eventSiteId + '']}
                        onChange={({option}) => {
                            console.log('option=====', option)
                            setSlot([option] as any)
                        }}
                    />
                </div>
            }
