...
import {getGroupMembers, Membership, Profile} from "@/service/solas";

function HomeUserPanel(props: {membership: Membership[], isSide?: boolean}) {
    ...
    return <div className={`home-user-panel ${props.isSide ? 'side': '' }`}>
               ...
               <div className={'group-card'}>
                   {
                       process.env.NEXT_PUBLIC_SPECIAL_VERSION === 'maodao' ?
                           <Link href={`/group/${eventGroup.username}`} className={'center'}>
                               ...
                               <svg className={'right'} xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" fill="none">
                   ...
                   <div>
                       {
                           props.membership.filter(item => item.role !== 'member').map((membership, index) => {
                               return <Link key={membership.profile.id}
                                    ...
                                    <img className={'avatar'} src={membership.profile.image_url || defaultAvatar(membership.profile.id)} alt=""/>
        ...
        <div className={'center'}>
            <Link href={`/event/${eventGroup?.username}/schedule`} className={'calendar-btn'}>
