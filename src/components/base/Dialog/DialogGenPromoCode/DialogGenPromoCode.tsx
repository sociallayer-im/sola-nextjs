...
export default function DialogGenPromoCode(props: {
    ...
    const handleShowList = async () => {
        ...
        try {
            ...
            openDialog({
                content: (close: any) => <DialogListPromoCode promoCodes={promoCodes} close={close}/>,
                size: ['100%', '100%']
            })
        } finally {
            unload()
    ...
    const addOne = () => {
        setPromoCode({
            ...promoCode,
            max_allowed_usages: promoCode.max_allowed_usages + 1
        })
    }
    ...
    const handleGenerate = async () => {
        ...
        if (promoCode.max_allowed_usages <= 0) {
        ...
        setBusy(true)
        try {
            const _promoCode = JSON.parse(JSON.stringify(promoCode))
            const event = {
                ...props.event,
                promo_codes: [...props.promoCodes, promoCode],
                ...
                <div className={`${styles['row']} ${styles['item']}`} style={{marginTop: '30px'}}>
                    ...
                    <div className={styles['input']} style={{width: '160px'}}>
                        <div className={styles['date-picker']}>
                            ...
                            <div>{dayjs(promoCode.expiry_time).format('MMM DD, YYYY')}</div>

                            <DatePicker
                                minDate={new Date()}
                                value={new Date(promoCode.expiry_time)}
                                onChange={({date}) => {
                                    setPromoCode({
                                        ...promoCode,
