...
import solas, {Group, Profile, ProfileSimple, searchDomain} from '../../../../service/solas'
...
export interface AddressListProps<T> {
    handleClose?: () => any
    handleConfirm?: (selected: ProfileSimple[]) => any
    value: ProfileSimple[]
    onChange?: (selected: ProfileSimple[]) => any
    title?: string
    confirmText?: string,
    single?: boolean,
}
...
function DialogAddressList<T>(props: AddressListProps<T>) {
    ...
    const addVale = (item: Group | Profile) => {
        const index = selected.findIndex((p) => p.username === item.username)
        const value = {
            id: item.id,
       ...
       <div className='top-side'>
           ...
           { !!searchKeyword.length &&
               <div className={'user-search-result'}>
                   <div className={'center'}>
                       { searchResult.length ?
                           <AddressList selected={ selected.map(p=> p.username!) as any } data= { searchResult } onClick={(profile) => { addVale(profile)} } />
           ...
           { !searchKeyword.length &&
               <AppTabs renderAll styleOverrides={ overrides } initialState={ { activeKey: "recently" } }>
                   <Tab key='recently' title={lang['Follow_detail_Recently']}>
                       <div className='center'>
                           { records.length ?
                               <AddressList selected={ selected.map(p=> p.username!) as any } data= { records } onClick={(profile) => { addVale(profile)} } />
                               : <Empty text={'no data'} />
                           }
                       </div>
                   </Tab>
                   <Tab key='follower' title={lang['Follow_detail_followed']}>
                       <div className='center'>
                           ...
                           <AddressList selected={ selected.map(p=> p.username!) as any } data= { followers } onClick={(profile) => { addVale(profile)} } />
                       </div>
                   </Tab>
                   <Tab key='following' title={lang['Follow_detail_following']}>
                       <div className='center'>
                           { followingsEmpty && <Empty text={'no data'} /> }
                           <AddressList selected={ selected.map(p=> p.username!) as any } data= { followings } onClick={(profile) => { addVale(profile)} } />
                       </div>
                   </Tab>
               </AppTabs>
           }
