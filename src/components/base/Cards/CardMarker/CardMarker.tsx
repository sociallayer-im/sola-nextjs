...
function CardMarker(props: { item: Marker, checkin?: MarkerCheckinDetail[] }) {
    ...
    useEffect(() => {
        if (props.checkin && props.checkin.length) {
            const target = props.checkin?.filter((item: MarkerCheckinDetail) => item.profile.id === user.id)
            setHasCheckin(!!target?.length)
        }
    }, [props.checkin])
    ...
    return (<div className={styles['marker-card']} onClick={e => {
        ...
        <div className={styles['left']}>
            ...
            <div className={styles['des']}>{props.item.message}</div>
