...
function CreateBadgeWithPrefill(props: CreateBadgeWithPrefillProp) {
    ...
    const send = async (acceptorDomain: string) => {
        ...
        try {
            ...
            unload()
            router.push(`/issue-success?gift=${badgelets[0].id}`)
        } catch (e: any) {
    ...
    const handleCreate = async () => {
        if (presetAcceptor) {
            ...
            router.push( `/issue-gift/${props.badgeId}?reason=${encodeURI(reason)}`)
        }
    }
            ...
            {  !!preFillBadge &&
                <div className='create-gift-page'>
                    <div className='create-badge-page-wrapper'>
                        <PageBack title={ lang['Create_Gift_Title'] }/>
