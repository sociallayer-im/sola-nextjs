...
const timezoneList = [
    ...
    ["Africa/Casablanca", "Casablanca", "GMT+1:00"],
    ["Africa/Harare", "Harare", "GMT+2:00"],
    ["Africa/Johannesburg", "Pretoria", "GMT+2:00"],
    ...
    const target = dit.find(item2 => {
        return item2.tzCode === item[0]
    })

    const offset = target ? target.utc : ''
    const cmtStr = offset ? `GMT${offset}` : `GMT+0`
